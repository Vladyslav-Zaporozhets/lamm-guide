...
import { Search, BookOpen, ChevronRight, ChevronDown, Folder, FolderOpen, Image as ImageIcon, ChevronLeft } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 -mt-10 relative z-20">
         ...
         {!searchQuery ? (
            <div className="bg-white rounded-3xl p-6 md:p-10 shadow-xl border border-slate-200">
               ...
               {path.length > 0 && (
                  <button onClick={navigateUp} className="mb-6 flex items-center text-blue-600 font-bold hover:text-blue-800 transition-colors bg-blue-50 px-4 py-2 rounded-xl text-sm">
               ...
               {/* Render Products for Current Level (If any exist exactly in this category node) */}
               {(() => {
                  // If we are at a specific node, show its products
                  if (path.length > 0) {
                                    ...
                                    <Link key={product.id} href={`/encyclopedia/${product.id}`} className="group flex flex-col bg-slate-50 rounded-2xl border border-slate-200 hover:border-blue-400 hover:shadow-xl hover:shadow-blue-500/10 transition-all overflow-hidden h-full">
                                       ...
                                       <div className="p-5 flex-grow flex flex-col justify-between bg-white">
                                          ...
                                          <div className="inline-flex items-center text-xs font-bold text-blue-600 mt-2 group-hover:translate-x-1 transition-transform">
                                             {t('readArticle')} <ChevronRight className="w-3 h-3 ml-1" />
                                          </div>
                                       </div>
                                    </Link>
                                 ))}
